# setup_chain_rr.R — Phase D: Setup, Chain, R:R (Steps D.1 – D.4)
#
# Runs only on Phase C survivors (~5-15 names). Each step has a single role:
#   D.1 vehicle + expiry choice
#   D.2 structural target consensus from prior swing high + 52w high + round#
...
compute_structural_target <- function(price, hist_close, hist_high) {
  ...
  # 1. Prior monthly swing high — highest local-max in last 60-120 days that
  # is above current price and has not been breached afterwards.
  ...
  for (i in 21:(length(recent_highs) - 5)) {
    ...
    if (is_local_max && recent_highs[i] > price) {
      # Unbroken: not exceeded by any close after position i
  ...
  # 3. Tiered round number above
  round_target <- .nearest_round_above(price)

  # Cluster the three sources — pick the two tightest that are above price
  ...
  # Find the pair with the tightest relative spread; prefer the lower pair
  # if multiple ties (lower target = more reachable).
  spreads <- diff(candidates) / candidates[-length(candidates)]
  # If smallest spread <= 5%, the two are "agreeing"
  ...
  if (length(candidates) >= 3 && spreads[best_idx] <= 0.05) {
    # Check if a third candidate is also within 5% of the cluster
    if (length(spreads) >= best_idx + 1 && spreads[best_idx + 1] <= 0.05)
      agreeing <- 3L
  }

  # Fib confirmation overlay — compute 1.272 and 1.618 extensions off the most
  # recent significant impulse leg. Heuristic: leg = (lowest in last 60d) →
  # (highest after that low). Fib confirms if extension is within ±2% of either
  # cluster target.
  fib_confirms <- FALSE
...
#' Per-strike chain OI walk (Step D.3). Reads from option_chain_oi_history
#' median over last 5 trading days (anti-noise N.2). Live walk via Tdata::
#' get_chain_oi() should be done by a separate scheduled task that populates
#' the table; this function does NOT call IBKR directly.
#'
#' @param sym character
#' @param expiry character "YYYYMMDD"
#' @param spot_target_low numeric
#' @param spot_target_high numeric
#' @param current_price numeric
#' @param conn DBI connection
#' @return list with all D.3 outputs
walk_chain_oi <- function(sym, expiry, spot_target_low, spot_target_high,
                          current_price, conn) {
  cutoff <- format(Sys.Date() - 7, "%Y-%m-%d")  # 5 trading days ~= 7 cal days
  ...
  # Concentration between current and target_low
...
#'
#' @param vehicle "call" | "spread" | "stock"
#' @param strike numeric — chosen strike (vehicle leg or stock = NA)
#' @param expiry character "YYYYMMDD"
#' @param current_price numeric — spot
#' @param effective_target numeric — Step D.3 binding spot target
#' @param iv_now numeric — current IV (5-day median, anti-noise N.1)
#' @param entry_premium numeric — current vehicle mid (for option vehicles)
#' @param spread_short_strike numeric — for spread, the short leg strike (NA otherwise)
#' @param risk_free numeric
#' @param rr_min numeric
#' @return list: rr, entry_floor, entry_ceiling, headroom_band, reward
compute_rr_entry <- function(vehicle, strike, expiry, current_price,
                             ...
                             rr_min = 0.5) {
  if (vehicle == "stock") {
    # Stock direct: risk = stop-distance, reward = effective_target - entry
    # Default stop = 99% of current MA50 — caller passes via entry_premium
    # repurposed as stop_distance for stock vehicle.
    risk <- entry_premium  # stop distance ($)
    ...
    return(list(rr = round(rr, 2),
                ...
                entry_ceiling = round(current_price, 2),  # stock has no premium ceiling
  ...
  if (vehicle == "call") {
    ...
    # Entry_Ceiling: solve P* such that (fwd_price - P*) / P* = rr_min
    # → P* = fwd_price / (1 + rr_min)
  ...
  if (vehicle == "spread") {
    # Debit call spread: long at strike, short at spread_short_strike.
    ...
    fwd_spread <- min(fwd_spread, max_payoff)  # capped
    risk <- entry_premium
...
#' Determine Entry_State (Step E.1, but logic lives here for unit testability)
