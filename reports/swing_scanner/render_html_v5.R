# render_html_v5.R — Interactive HTML report with DataTables, per-day file
#
# Renders the v5 scanner output as a single self-contained HTML file:
...
render_scanner_html_v5 <- function(df, funnel, out_dir, run_date = Sys.Date(),
                                   rr_min = 0.5) {
  if (!requireNamespace("jsonlite", quietly = TRUE))
    stop("jsonlite required for render_scanner_html_v5")
  ...
  fname <- sprintf("swing_scanner_v5_%s.html", format(run_date, "%Y%m%d"))
...
<title>Swing Scanner v5 — %s</title>
