...
#
# Schema version is an internal constant (SCANNER_SCHEMA_VERSION) written
# alongside each persisted row, not encoded in filenames.
...
to_persist$schema_version <- SCANNER_SCHEMA_VERSION
to_persist$fib_confirms <- as.integer(to_persist$fib_confirms)
to_persist$crowded_flag <- as.integer(to_persist$crowded_flag)
tryCatch({
  dbExecute(conn, "DELETE FROM scanner_results WHERE cache_date = ?",
            params = list(.today))
  dbWriteTable(conn, "scanner_results", to_persist,
               append = TRUE, row.names = FALSE)
}, error = function(e) message("scanner_results persist failed: ", e$message))
dbDisconnect(conn)
...
csv_file <- file.path(out_dir, sprintf("swing_scanner_%s.csv",
                                       format(Sys.Date(), "%Y%m%d")))
df$schema_version <- SCANNER_SCHEMA_VERSION
write.csv2(df, csv_file, row.names = FALSE)
message(sprintf("CSV written: %s", csv_file))

html_file <- render_scanner_html(df, funnel, out_dir,
