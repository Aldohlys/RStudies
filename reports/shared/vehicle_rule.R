...
pick_vehicle_expiry <- function(price, cheap_score, stage,
  ...
  vehicle <- if (price < 10) "stock"
             ...
             else if (price < 150 && cs >= 7) "call"
             else "spread"

  reason <- switch(vehicle,
    stock  = if (price < 10) sprintf("price $%.2f < $10 (stock vehicle)", price)
             else sprintf("ATM bid-ask %.1f%% > 8%% (option spread too wide)",
                          atm_bid_ask_pct),
    call   = sprintf("price $%.2f < $150 and cheap_score %d ≥ 7 (outright call)",
                     price, as.integer(cs)),
    spread = sprintf(
      "price $%.2f%s (vertical spread for IV cost control)",
      price,
